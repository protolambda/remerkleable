...
def hash_tree_root(v: View) -> Root:
    return Root(v.get_backing().merkle_root(merkle_hash))
...
privkeys = [i + 1 for i in range(512)]
pubkeys = [bls.privtopub(privkey) for privkey in privkeys]

def build_mock_validator(i: int, balance: int):
    pubkey = pubkeys[i]
    # insecurely use pubkey as withdrawal key as well
    withdrawal_credentials = BLS_WITHDRAWAL_PREFIX + hash(pubkey)[1:]
    return Validator(
        pubkey=pubkeys[i],
...
def create_genesis_state(validator_balances, activation_threshold):
    ...
    state.validators = [build_mock_validator(i, state.balances[i]) for i in range(len(validator_balances))]
